...
| `onTapToFocus`                  | Function                               | Callback when the user taps the preview to focus, fired alongside the built-in tap-to-focus so an overlay can be shown without suppressing focus and zoom. Returned event contains `x` and `y`, normalized 0-1 in preview space. Requires `focusMode` `'on'`. Ex: `onTapToFocus={(e) => console.log(e.nativeEvent.x, e.nativeEvent.y)}`.                                                                                                                   |
...
| **iOS only**                    |
| `ratioOverlay`                  | `'width:height'`                       | Show a guiding overlay in the camera preview for the selected ratio, expressed as `width:height`. The largest area of that ratio is centred in the preview and the rest is masked, so `'4:5'` is taller than it is wide and `'16:9'` is wider than it is tall. Does not crop image as of v9.0. Examples: `'16:9'`, `'1:1'`, `'4:5'`                                                                                                                                                                                                                                                                                                                                     |
